...
describe("Blog app", () => {
  ...
  describe("When logged in", () => {
    ...
    test("a new blog can be created", async ({ page }) => {
      ...
      await expect(addedDiv).toHaveCSS(
        "background-color",
        "rgb(211, 211, 211)",
      );
      await expect(addedDiv).toHaveCSS("font-size", "20px");
      await expect(addedDiv).toContainText(
        "a new blog TestTitle by TestAuthor added",
      );
    ...
    test("only the creator of a blog can see the remove button", async ({
      page,
    }) => {
      await page.getByRole("button", { name: "create new blog " }).click();
      await page.getByTestId("title").fill("TestTitle");
    ...
    test("blogs are sorted in descending order by likes", async ({ page }) => {
      ...
      await blogDiv
        .getByText("TestTitle TestAuthor view")
        .locator("..")
      ...
      await blogDiv
        .getByText("TestTitle2 TestAuthor2 view")
        .locator("..")
        .getByRole("button", { name: "view" })
        .click();
      await blogDiv
      ...
      await blogDiv
        .getByText("TestTitle3 TestAuthor3 view")
        .locator("..")
        .getByRole("button", { name: "view" })
        .click();
      await blogDiv
